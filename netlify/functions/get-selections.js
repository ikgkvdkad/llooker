...
exports.handler = async (event) => {
  ...
  try {
    const listQuery = {
      text: `
        SELECT id, created_at, captured_at, role, image_data_url
        FROM ${CAMERA_SELECTIONS_TABLE_NAME}
    ...
    const countResult = await pool.query(`SELECT COUNT(*) AS total FROM ${CAMERA_SELECTIONS_TABLE_NAME}`);
    ...
    console.error('Failed to fetch camera selections:', {
      message: error?.message,
      stack: error?.stack
    });
    return {
      statusCode: 500,
      body: JSON.stringify({ error: 'Failed to load selections.' })
    };
  }
};
